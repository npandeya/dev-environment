...
ssh -i "$PEM_FILE" "$BASTION_HOST" << 'EOF'
  echo $HOSTNAME
  helm list --all-namespaces --short | grep -v 'aws-load-balancer-controller' | grep -v 'aws-ebs-csi-driver' | xargs -I{} sh -c 'helm uninstall {} --namespace $(helm list --all-namespaces | grep {} | awk "{print \$2}")'
EOF

echo "All helms destroyed now its turn for the aws resources!"
echo 
echo "Destroying all resources...but sleeping 300 seconds before that."
for ((i=1; i<=300; i++)); do
  printf "${RED}${BOLD}Waiting %02d seconds... Press Ctrl+C to cancel.${NC}\r" "$i"
  sleep 1
done
